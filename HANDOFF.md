...
> 记录截至 **v1.0.58**（versionCode 63，2026-09-25）的全部工程知识。
...
- 全量构建约 2~3 分钟；**394 条单测**必须全过才算交付（其中 9 条用 Robolectric，见 §8 S1）
