...
│   ├── db/         # AppDatabase（Room，version=14）、DAO
│   ├── entity/     # Room 实体
...
- **测试基线**：244 条单测全绿；新增功能须同步补测（`app/src/test/.../`，20 个测试文件覆盖 backup / 加密（v2+v3）/ 附件路径与远端比对 / PROPFIND 解析 / 停药警示与三态 / 化验单位分组 / 筛查种子 / 补剂上限与错开 / 通用名键 / 运动分级 / 导入解析 / 排程计算 / 知识库检索 / 紧急卡用药汇总 / 恢复码 / 复诊准备 / 漏服 / 体重目标）
