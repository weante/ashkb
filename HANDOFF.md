...
| v1.0.39 | ①「数据」页出现「推荐食谱」入口 → 进入见 10 条食谱，可按「抗炎 / 胃肠友好 / 控热量」筛选 ②点开一条食谱 → 详情见配料 / 做法 + 出处（编号 + 完整题录）+ 免责声明；**列表页不显示出处正文** ③收藏一条 → 置顶；自建一条 → 显示「自建」徽标且详情无出处小节 ④运动页出现「康复计划」入口 → 见 4 / 8 / 12 周三个模板 ⑤点「添加模板」→ 首次「已添加 3 个模板」，再点「模板已齐全」⑥启用 4 周计划 → 显示「第 1 / 4 周」「本周 x / 3 天」「累计 x / 16 天（y%）」；打卡运动后数字增加 ⑦旧备份恢复后数据完整（新表为空） | ☐ |
