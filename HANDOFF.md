...
> 记录截至 **v1.0.46**（versionCode 51，2026-09-22）的全部工程知识。
...
| v1.0.46 | ①~⑦ 同上行 v1.0.45 的全部条目，另加：⑧**拖动命中**：在日期间隔不均的图上（前密后疏）拖动，读数气泡应始终跟在**手指下方最近的点**上（旧版会跳到远处）⑨**入场动画**：切到趋势页后折线从左往右扫出，**末点数值气泡应在折线扫到末点后才出现**（旧版提前悬浮）⑩快速连点「近 7 天 → 近 30 天」：图最终显示的必须是 30 天的数据（旧版可能停留在 7 天结果）⑪旧备份恢复后数据完整（无结构变更） | ☐ |
