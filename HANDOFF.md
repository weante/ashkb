...
> 记录截至 **v1.0.53**（versionCode 58，2026-09-24）的全部工程知识。
...
- 全量构建约 2~3 分钟；**348 条单测**必须全过才算交付（其中 9 条用 Robolectric，见 §8 S1）
...
- **测试基线**：**348 条单测全绿**；新增功能须同步补测（`app/src/test/.../`，**31 个测试文件**覆盖 backup / 加密（v2+v3）/ 附件路径与远端比对 / PROPFIND 解析 / 停药警示与三态 / 化验单位分组 / 筛查种子 / 补剂上限与错开 / 食谱种子与出处台账 / 计划模板编解码 / 计划完成度 / 通用名键 / 运动分级 / 导入解析 / 排程计算 / 知识库检索 / 紧急卡用药汇总 / 恢复码 / 复诊准备 / 漏服 / 体重目标 / **服药依从口径与用药记录修正不变量 / 注射部位存库键稳定性 / ISO 时刻与时间选择器口径（`hhmm`、`timeParts`、槽位标签 = 计划时刻）/ 崩溃留档凭据脱敏 / 源码正则花括号静态守卫 / 提醒链「取消-重建」语义（Robolectric）**）
...
| v1.0.53 | ⚠️ **本版改了提醒调度的生产代码（只加了带默认值的 `now` / `today`），务必重点回归提醒链路**：①**到点提醒**：设一支药计划时刻为 2~3 分钟后 → 到点应正常收到通知 ②**升级重查**：到点不打卡 → +30 分钟应再次提醒（上限 2 次）③**打卡后不再提醒**：到点前打卡 → 到点与 +30 都应静默无通知 ④**改时刻**：把某药时刻从 A 改到 B → B 到点应提醒；A 的旧闹钟若仍残留，触发时应**静默取消**（不发通知）⑤**停药**：停用一支药 → 不再收到该药任何提醒 ⑥**重启手机**：重启后当天未打卡的槽位仍应提醒（含未到点的 +30 / +60）⑦**冷启动 / 打卡 / 改药单后**，当天后续的升级重查不应被清掉 ⑧顺带确认 v1.0.52 的时间选择器、v1.0.50 的「已服 HH:mm」、v1.0.49 的用药记录修正仍正常 ⑨旧备份恢复后数据完整（无结构变更） | ☑ **已验（2026-09-24 用户确认「转」）**，**已转正为当前正式版（Latest）** |
