...
> 记录截至 **v1.0.51**（versionCode 56，2026-09-23）的全部工程知识。
...
| v1.0.51 | ①**注射类能设计划用药时间**：我的 → 药单 → 添加（或编辑）一支**注射**药 → 选频次（如「每两周一次」）→ 应出现**「计划用药时间（到点提醒）」**并可选 06:30 / 08:00 / 12:00 / 18:00 / 21:00 或自定义 → 保存 ②**注射单选**：再点已选中的时刻不应取消（避免变成「零个时刻」）③**今日卡显示时刻**：注射日当天，今日卡上应显示该**计划时刻**（如「08:00」），旁边仍有「注射」标签；**不应只有一个「注射」看不到时刻** ④**口服仍可多选**：编辑一支口服药（如每日两次）→ 仍能选 08:00 + 20:00，并显示「已选时刻：08:00、20:00」⑤**自定义时刻校验**：输入 `25:99` 点「添加」应不生效（此前会被加进去但存库后被过滤，等于假生效）⑥**文案**：注射表单里不应再写「默认 09:00 提醒」⑦用药记录修正 / 已停用药品（v1.0.49/48 项）仍正常 ⑧今日页「已服 HH:mm」仍正常（v1.0.50 项）⑨旧备份恢复后数据完整（无结构变更） | ☐ |
