...
> 记录截至 **v1.0.50**（versionCode 55，2026-09-23）的全部工程知识。
...
| v1.0.50 | ①**今日页「已服」后的时刻**：打卡一条药 → 今日页该卡应显示「已服 **HH:mm**」（如「已服 22:31」）——**不应再出现「已服 19981」这类一串数字**（旧版把小数秒当成了时刻）②**晚于计划**：若晚于计划 30 分钟以上，后面仍应跟着「（晚于计划 30 分钟以上）」③**补剂历史时刻**（营养页点补剂 → 近 90 天记录）应仍显示「2026-09-23 22:31」形式（该处走 `take(16)`，本就正常，顺带确认没被改坏）④用药记录修正 / 已停用药品（v1.0.49 / v1.0.48 项）仍正常 ⑤打分滑杆拖动（v1.0.47 项）仍正常 ⑥旧备份恢复后数据完整（无结构变更） | ☐ |
