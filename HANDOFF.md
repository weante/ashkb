...
| v1.0.53 | ⚠️ **本版改了提醒调度的生产代码（只加了带默认值的 `now` / `today`），务必重点回归提醒链路**：①**到点提醒**：设一支药计划时刻为 2~3 分钟后 → 到点应正常收到通知 ②**升级重查**：到点不打卡 → +30 分钟应再次提醒（上限 2 次）③**打卡后不再提醒**：到点前打卡 → 到点与 +30 都应静默无通知 ④**改时刻**：把某药时刻从 A 改到 B → B 到点应提醒；A 的旧闹钟若仍残留，触发时应**静默取消**（不发通知）⑤**停药**：停用一支药 → 不再收到该药任何提醒 ⑥**重启手机**：重启后当天未打卡的槽位仍应提醒（含未到点的 +30 / +60）⑦**冷启动 / 打卡 / 改药单后**，当天后续的升级重查不应被清掉 ⑧顺带确认 v1.0.52 的时间选择器、v1.0.50 的「已服 HH:mm」、v1.0.49 的用药记录修正仍正常 ⑨旧备份恢复后数据完整（无结构变更） | ☐ |
