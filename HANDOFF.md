...
| v1.0.40 | ①**冷启动不崩**（v1.0.39 闪退修复）②「数据」→「推荐食谱」可用、10 条、筛选/收藏/自建正常 ③「运动」→「康复计划」可用、三模板 + 启用后进度正常 ④若本次启动仍异常，首屏会弹一条「上次启动异常：…」摘要（截图发给开发者）；完整堆栈在 `filesDir/last_crash.txt` | ☐ |
