...
| v1.0.42 | ①**今日 → 今日运动 → 康复计划 → 添加模板**：提示「已添加 3 个模板」且**不闪退** ②再点一次：「模板已齐全」（幂等）③启用某模板后，完成度随运动打卡变化 ④旧备份恢复后数据完整 | ☑ **已验（2026-09-21 用户确认「已添加，不闪退」）**，已转正式发布 + Latest；②③④ 未逐条实测（低风险，后续顺带看） |
