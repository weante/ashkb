...
> 记录截至 **v1.0.67**（versionCode 72，2026-09-26）的全部工程知识。
...
- 全量构建约 2~3 分钟；**471 条单测**必须全过才算交付（其中 31 条用 Robolectric，见 §8 S1）
...
- **⚠️ 小多图的横轴必须由「共享窗口」决定，不能各格按自己的数据范围铺开（v1.0.54）**：小多图（small multiples）的全部价值在于**同一时间点上下对齐着看**——「疼痛先动还是血沉先动」这类问题只能靠对齐回答。若每格各自 min–max 铺满 0%–100%，各格同一横坐标对应的日期就不同，叠着看会得出**错误结论**，而**图画出来依然很好看**（又是「沉默的谎言」那一类）。做法：窗口 = 全部序列日期的并集（`sharedWindow`），相对坐标 = `(日期 − 窗口起) / 窗口长度`（`xFraction`），两者都是纯函数、单测覆盖（含单日窗口除零 → 0.5、跨月、越界夹紧、脏日期 → null 跳过）。**通用判据**：凡是「把多个序列放在一起比较」的图，先问一句「各图的坐标轴是不是同一把尺子」。
- **⚠️ 自由文本字段做「按类取数」必须先归一，且宁可漏配不要配错（v1.0.54）**：`lab_results.test_name` 没有编码列，`observeTrend` 又是精确等值匹配，于是 `"血沉(ESR)"` 与 `"ESR"` 会变成两条互不相干的序列。修法是加一层显式目录（`LabIndicator.aliases`）+ 归一（大小写 / 全角括号 / 各类空白），**刻意不用 `contains` 模糊匹配**：配错的后果是把别的指标画进来（静默画错），漏配只表现为「暂无数据」（可被发现）。同理**单位**必须显式换算或排除并计数——CRP 的 `mg/dL` 与 `mg/L` 差 10 倍，混画会凭空多出一次「骤降」；**「缺失」与「认不出」要分开计数并在 UI 说明**，否则用户会把「图上没有」读成「没测过」。
- **⚠️ 靠枚举「写法」永远会漏，要用有界规则覆盖整类变化（v1.0.55 用户实测）**：v1.0.54 的别名表枚举了「血沉」「血沉(ESR)」「血沉测定」等，却漏了用户真实数据里的**「红细胞沉降率测定」**（复诊管理→化验里显示的正是这个名字），于是趋势页**一个点都认不出来**。教训：**枚举具体字符串是「用举例代替规则」**，样本之外必失效。改法：归一（大小写 / 全角括号 / 空白）→ 逐层剥掉**尾限定词**（`测定`/`定量`/`检测`/`检验`/`检查`/`试验`/`法`）与**去括号及其内容** → 逐个有限形态去比别名表。关键是**仍要命中显式别名表**，所以「有界变形」不等于「模糊匹配」，不会把别的指标卷进来（单测同时锁两侧：该认的必须认、不该认的一个都不认）。
- **⚠️ 共享时间轴的前提是「采样频率可比」（v1.0.55 补 v1.0.54）**：v1.0.54 把所有指标放进同一根共享轴，对「每日/每周记录」是对的；但**化验是几个月一次**，与日常记录差 1~2 个数量级，同轴必然压扁一方（化验挤成右侧一个点，或日常指标压成左侧一条线）。修法是**分区各自一根共享轴**：日常 6 格一根（跟随 7/30/90 天），炎症 2 格一根（不设界）。**判据**：把序列放进同一根轴之前，先问「它们的采样频率在同一量级吗」；不在就先分组。同时这也说明——**别把「一个窗口」当成全局设定**，不同性质的数据本就该有各自的窗口。
- **⚠️ 同一蛋白的不同检测不要合并；量级不同的两项永远不要画成一条线（v1.0.56 实测事故）**：`CRP` 与 `超敏C反应蛋白 hs-CRP` 是同一蛋白，v1.0.54 因此把 hs-CRP 的写法并进了 CRP 序列——**结果画出的是错数**：同一份化验单里 CRP 36.33 mg/L（偏高、在「异常项」里）与 hs-CRP 0.4 mg/L（正常、在折叠的「正常项」里）**同一天**各一条，按日期去重时选中了 hs-CRP 的小数值，于是**图上 CRP = 0.4、化验单上 36.33**，而 hs-CRP 那条藏在折叠区，用户极难察觉异样。**教训两条**：①「同一个东西」不等于「同一个测量」——量级差一个数量级的检测（常规 vs 高敏）**必须各自成序列**（v1.0.57 进一步按用户要求把 hs-CRP **整项移除**，这类记录不参与任何趋势）；② 去重/取优这类「从多条里挑一条」的逻辑，**只要可能挑错就必须把「挑了什么、丢了几条」说出来**（v1.0.56 加了 `sameDateConflict` 计数并在图下显示）。判据：凡「多条里选一条」的取舍，先问「我丢掉的那条，用户能看出来吗」。
- **⚠️⚠️「从多条里挑一条」这个动作本身可能就是错的——实在要挑，先考虑能不能全都留下（v1.0.57 定稿，连修三版才到位）**：用户 2026-03-13 有**两条 CRP**（36.33 与 0.4）。v1.0.54 猜成因错方向（以为是 hs-CRP 混入）；v1.0.56 拆开 hs-CRP 并把「同日不同值」计数**如实显示**出来——**提示确实生效了**（截图里出现「同日有 1 条不同数值，已取最近录入的一条」），**但数值仍然是错的**（0.4）。因为那条「同日保留最近录入的一条」的规则一直在。**只把「我们丢了几条」说出来，并没有把「丢掉的数值」还回来**——用户要的是那个数，不是一句说明。最终修法：**同日多值全部画出，一个不丢**（完全相同值才合并）。**通用判据（比上一条更强）**：遇到「多条冲突、只能取一条」时，先问「能不能不取？把冲突原样呈现出来是不是更好？」——**用户的数据决定让用户自己做**；只有当「全都留下会让人误读」时才退而取一，且必须说明取了哪条、丢了几条。
...
- **⚠️ 一个 x 对应多个 y 后，「先用 x 定位、再取第 i 个点」的代码全都会挑错——画好了不等于读数也对了（v1.0.58 修 v1.0.57）**：v1.0.57 让同日多值**全部画了出来**（图上一段竖线），但**拖动读数仍只报一条**，且报的恰是较小的 0.4（用户实测「手指拖动到 3.13 还是显示 0.4」）。原因：`nearestIndex` 按 `d < bestDist`（**严格小于**）取最近点，而同日的点 x 完全相同 ⇒ 命中**第一个**下标；又因同日多值在 `points` 里按数值升序 ⇒ 永远是最小的那条。**教训**：把绘制从「一对一」放宽成「一对多」之后，**所有消费方（绘制 / 命中 / 读数气泡 / 末点去重判断……）都得同步改**，否则会留下一半对一半错的混合状态，且**看起来像修好了**（线画对了）。改法：新增 `sameDateIndices(points, index)` 作为「按 x 取点」的唯一口径（返回该日**全部**下标），绘制与读数共用；气泡文案 `selectionText` 同日多值全部列出且从大到小。**通用判据**：数据结构从「一对一」放宽到「一对多」时，先 grep 所有「按 key 取单值」的调用点。
- **⚠️ 对「图上已经看得到」的事实再加一行文字，只会撑高 / 搞乱布局（v1.0.58）**：v1.0.57 在小图下加了一行「有 N 个日期存在多条不同数值（均已画出）」，但那些值本就已全部画出、一段竖线已把话说清；多挂这行字把该格撑高、与左侧格不齐，破坏了 2 列小多图的整齐（用户实测反馈）。**判据**：文字补注只应用于「**图上看不出来的事**」（如 v1.0.54 的「另有 N 条因单位不同未纳入」——被排除的数据确实看不见）；**能被图形本身表达的，就不要再写一遍**，尤其在「整齐本身就是功能」的小多图布局里。
- **Robolectric 改不动 `java.time` 的挂钟（v1.0.53 实测）**：`ShadowSystemClock` 只管 `android.os.SystemClock`；`ShadowSystemClock.advanceBy(...)` 之后 `LocalDateTime.now()` **保持不变**（实测 `expected:<12> but was:<23>`），且该类没有 `setCurrentTimeMillis`。「现在」不可控 ⇒ 凡行为依赖当下的被测对象，**测试要么注入「现在」，要么就会漂移**。做法：给 `rescheduleAll(..., now: LocalDateTime = LocalDateTime.now())` 与 `cancelAllFuture(..., today: LocalDate = LocalDate.now())` 加**带默认值**的参数（对生产调用方零影响），测试传固定值。这与 `ScheduleCalc.slotsFor(med, date)` 显式传日期的既有风格一致。
- **⚠️ 不要用 `assumeTrue` 静默跳过时间敏感测试（v1.0.53 自己踩到）**：第一版用 `assumeTrue(LocalDateTime.now().hour in 1..22)` 规避跨零点，结果 23:06 再跑时**整类 9 条全部 skipped**——套件的 `skipped` 计数悄悄从 0 变 9，而「测试全绿」的结论看起来毫无异常。**回归锁一旦静默失效就等于没有**（比红灯更危险）。正确做法是把时间前提**消掉**（注入「现在」），而不是把不满足前提的用例藏起来；确实无法消除时，宁可用**会失败的断言**（本例第一版的 `assertEquals(12, ...)` 就成功地把「时钟推不动」暴露成 9 条失败，而不是 9 条跳过）。
- **⚠️ Robolectric 能注入触摸，但驱动不了 M3 `Slider` 的拖动（v1.0.53 实测证伪）**：S1b 计划用 Robolectric + `compose-ui-test` 做滑杆手势回归，写完 4 条后有 3 条失败（`swipeRight/swipeLeft/click` 对 `ScoreInput` 的值**零影响**）。用两个探针定位：① 最小 `pointerInput` 盒子**能**收到注入的 down（`downs>0`）→ 触摸注入本身可用；② 把**裸 M3 `Slider`**（无 `ScoreInput` 包装、无祖先旁听）放同一环境拖，**同样拖不动**（值恒为 0）。**结论：与 `ScoreInput` 写法无关，是环境能力边界**——那 3 条失败是**假红**，**不能**据此判定应用缺陷（否则会冤枉代码，或反过来用一个永远失败的测试把真问题淹没）。故 S1b 不落地，`build.gradle.kts` 就地记明。**方法论同上一条：判断「工具/环境能否承担某项验证」必须做对照探针，且探针要能区分「被测对象有问题」与「环境测不了」。**
- **⚠️ 小多图的横轴必须共享，且必须按「日期比例」而非「序号比例」（v1.0.54）**：小多图的全部价值在于「同一时间点上下对齐着看」。若每格按**自己的数据范围**铺开横轴，各格 0%–100% 对应的日期就各不相同，叠着看会得出错误结论——**而图画出来依然很好看**，属于典型的「静默说谎」（与 v1.0.45 那条「横轴必须由日期决定」同源，但这里多一层：**连窗口本身都必须先统一**）。做法：窗口由全部序列日期的**并集**算一次（`sharedWindow`）、全格共用；相对坐标按 `(日期 − 窗口起) / 窗口长度`（`xFraction`）。两个都单测：并集要忽略脏日期、单日窗口要防除零、越界要夹紧。
- **⚠️「图上没有」与「没测过」是两件事（v1.0.54）**：化验数据里有两类行**不会**进图——单位认不出（CRP 的 `mg/dL` 与 `mg/L` 差 10 倍，按错单位画会凭空多出一次「骤降」）与单位缺失。此时**既不静默丢弃、也不假装单位正确**：分别计数并在图下写明「另有 N 条…未纳入」/「另有 N 条无单位，按 X 计」。判据：**凡「因为不确定所以没画」的数据，都必须把它「存在过」这件事说出来**，否则用户会把沉默当成「没记录」。
...
- **⚠️ 不要从 ISO 时刻串的「尾部」截取时刻（v1.0.50 修复）**：`nowIso()` 用 `DateTimeFormatter.ISO_LOCAL_DATE_TIME` 写库，该格式**纳秒非零时会追加小数秒**，于是串长在 19 / 23 / 26 之间浮动（`2026-09-23T22:31:15` / `…15.199` / `…15.019981`）。今日页曾用 `takenAt.takeLast(5)` 从尾部截「HH:mm」，串长一浮动就截到小数秒——`…15.019981` → `19981`，界面上显示成「已服 19981」（用户截图报障）。**判据：取 ISO 时刻的时刻部分，要么解析（`ScheduleCalc.hhmm`），要么从头截（`take(16)` 取到分钟）；一律不要从尾截。** 另：这类缺陷**不报错、不影响逻辑**（`isLate` 走 parse 一直是对的），所以只能靠「看界面」发现——**凡是从结构化字符串里按固定位置截取的展示代码，都要问一句「这个串的长度是固定的吗」**。全库同类排查：`WellnessScreen` `take(16)` / `BackupScreen` `take(19)` / `AttachmentPath` `take(10)` 均为从头截（安全）；`TrendChart` 的 `takeLast(5)` 作用于纯日期串（无小数秒，安全）。
- **⚠️ 字段只在一条分支里渲染 → 另一条分支会被静默写入「表单默认值」（v1.0.51 修复）**：`MedEditScreen` 的「计划用药时间」选择器只写在 `route == "oral"` 分支里，而 `buildMed()` 对**所有**非 PRN 药都写 `takeTimes = times.take(1)`——`times` 是表单默认值 `["08:00"]`。于是**注射药的计划时刻被静默固定为 08:00，用户从未见过这个值、也无从修改**；表单里那句说明还写着「默认 09:00」，与真实值都不一致。**判据**：`buildMed()` 里出现的每个字段，都要能回答「用户在哪里能看到/改到它」；**只在部分分支渲染的输入控件，其对应的 `buildMed` 取值要么按分支给不同默认值并说明，要么让另一条分支也能编辑**。另：`PlanSlot.label` 是今日卡 chip 的显示文本，注射槽位把它写死成「注射」→ 计划时刻在全应用无处可见；**`label` 一律应为计划时刻**，槽位类型由 `key`（`"inj"`）与卡片上的给药途径 chip 承担。
- **⚠️ 不要用「固定候选 + 选中态」表达一个可自由取值的字段（v1.0.52 修复）**：「计划用药时间」曾用一排候选 chip（06:30 / 08:00 / …）的**选中态**表达当前值，另配一个「自定义 HH:mm」输入框。问题：**只要存的时刻不在那 5 个候选里（如 07:30），编辑页上没有任何 chip 被选中——用户看不到自己设过什么**，也就无法判断该不该改；「自定义」输入框还是空的（它只用于「新增」，从不回填）。**判据：可自由取值的字段，UI 必须把「当前值」当作文本直接显示出来，选择器只负责改它**；候选 chips 只适合「取值集合封闭且很小」的场景（如枚举、星期）。同类自查点：任何「用控件状态（选中/勾选/高亮）暗示当前值」的地方，都要问一句「值落在集合之外时会怎样」。
...
- **最新版**：v1.0.58（versionCode 63）：v1.0.21 知识库检索优化 + v1.0.22 清除 `as MutableStateFlow` 强转 + v1.0.23 化验 Tab 分组折叠 + v1.0.24 文案资源化 + v1.0.25 Compose 性能审查第一批 + v1.0.26 紧急卡「当前用药」+ v1.0.27 备份恢复码（v2 信封）+ v1.0.28 Compose 性能审查第二批 + v1.0.29 热修启动闪退 + v1.0.30 趋势图动画修复 + v1.0.31 药单在用药品编辑 + v1.0.32「我的」页关于卡 + v1.0.33 规划缺口第二批（Room v9→v10）+ v1.0.34 附件归档增强（Room v10→v11：化验/影像归属复诊记录）+ **v1.0.35 附件接入 WebDAV 备份（Room v11→v12 + 备份格式 v2→v3：稳定 vault key / 逐个加密上传 / 懒下载 / 同步删除 / 同步开关）** + v1.0.36 附件远端校验与补传（PROPFIND 列远端比对：远端缺失补传 / 远端多余清理；无结构变更） + **v1.0.37 用药精细化 + 化验与筛查（Room v12→v13：C5 原因枚举对齐 / C6 减量中态 + 医嘱减量豁免警示 / C2 补剂依从 / C3 化验按单位分组 + 跨院提示 / C10 生物制剂筛查与续方种子）** + **v1.0.38 营养素精细化 + 周月报（Room v13→v14：B11 补剂每日上限 + 与用药错开提醒 + 服药/补剂合并时间表；B4 报表第 4 页签「周月报」）** + **v1.0.39 推荐食谱库 + 周期康复计划（Room v14→v15：B3 `recipes` 表 + 10 条带出处种子；B7 `exercise_plans` 表 + 4/8/12 周模板 + 完成度反算）** ⚠️ **该版冷启动闪退、已废弃（勿发布）；功能随 v1.0.40 交付** + **v1.0.40 冷启动兜底 + 崩溃留档（两个新 VM 改为进页面才创建 + 启动期整体兜底 + CrashLogger；无结构变更）** ⚠️ **其「修复」不成立：只是把启动期异常吞掉，崩溃从「冷启动」挪到了「点添加模板」** + **v1.0.41 留档与兜底改进（`CrashLogger.recordNonFatal` + 崩溃摘要带 `Caused by`/栈帧 + 两条 proguard `-keep`）** ⚠️ **其「修复」不成立：归因 R8 是错的（`-keep` 与本根因无关）** + **v1.0.42 真正修复「康复计划 → 添加模板」闪退（真因：`ExercisePlanTemplates` 的正则末尾有一个未转义的 `}`——Java 的 `Pattern` 视为普通字符，Android 的 ICU 引擎抛 `PatternSyntaxException`，致类初始化失败 → 启动期被吞 → 二次触碰 `NoClassDefFoundError: K1.n`；修法：`parse` 移除正则改逐字符扫描 + `esc` 转义 `\n`/`\r`/`\t`；无结构变更）** —— **v1.0.38 / 39 / 40 / 41 均已被 v1.0.42 取代**（v1.0.39–41 为预发布且含缺陷，已在各 Release 页顶部标注；v1.0.38 功能全部含于 v1.0.42），**v1.0.42 已被 v1.0.43 取代**（功能全部含于 v1.0.43，无需再装本版） + **v1.0.43 代码审查后的 A / B 类修复 + 口令框「长按显明文」** + **v1.0.44 第三轮审查修复（N1 开机广播补 doneRefs + 删参数默认值 / N2 README 修正 / N3 种子版本化增量刷新 / S3 vault key 覆盖守卫 / S4 备份类型不符可定位报错 / S5 崩溃留档凭据脱敏；另落地正则花括号静态守卫）** + **v1.0.45 趋势页改造（方案 A）+ 7/30/90 天切换（X 轴按日期定位 / 刻度自适应 / 高度确定化 / 数据点与末点数值 / 阈值入轴槽 / 读数摘要行）** —— ⚠️ **v1.0.44 / v1.0.45 均为未验证预发布且各含缺陷（v1.0.45 的 4 处趋势图缺陷见 v1.0.46），两者均已被 v1.0.46 取代，请直接跳过** + **v1.0.46 审查修复（无结构变更：v1.0.45 的 4 处趋势图缺陷——拖动命中选错点 / 动画期间末点数值提前出现 / 阈值标签边缘溢出 / xAt 未排序防御；另 setTrendDays 过期响应竞态 + 正则守卫覆盖面告警 + 死资源清理）** ⚠️ **未真机验，已被 v1.0.47 取代** + **v1.0.47 修复打分滑杆「完全无法拖动」（无结构变更：删除 v1.0.17 引入的同级透明覆盖层——Compose 同级重叠只有 z 序最高者算命中，覆盖层独占了指针事件；点按旁听改挂祖先 Box；拖动判定改累计位移；旁听只补 M3 不响应的「点按落在当前值上」情形）** ⚠️ **未真机验，已被 v1.0.48 取代** + **v1.0.48 药单新增「用药记录」与「已停用药品」（无结构变更，仅新增只读查询：点药名看近 90 天逐条流水 + 该药依从率；底部折叠区显示停药日期/原因/备注并可点开记录；依从率公式由 ReportRepository 内联 4 遍收拢为 `domain/AdherenceCalc` 唯一实现）** + **v1.0.49 用药记录可手动修正 + 注射部位显示中文（无结构变更：记录行加铅笔可改状态/原因/注射部位/备注，归属日与计划时刻只读；注射部位键→中文映射由 TodayScreen 私有函数收拢为 `InjSite` 枚举并删掉 6 条死字符串；编辑不变量抽成 `domain/MedLogEdit`）** + **v1.0.50 修复今日页「已服」后显示一串数字（无结构变更：`nowIso()` 的 `ISO_LOCAL_DATE_TIME` 在纳秒非零时会追加小数秒，串长在 19/23/26 间浮动，而展示侧用 `takeLast(5)` 从尾部截「HH:mm」，截到的是小数秒——「已服 22:31」显示成「已服 19981」；新增 `ScheduleCalc.hhmm()` 改为解析后再格式化）** + **v1.0.51 注射类补上「计划用药时间」+ 口服字段改名（无结构变更：表单里时刻选择此前只在口服分支渲染，注射的时刻被静默写成默认 08:00 且全应用无处可见——今日卡 chip 显示的是 `PlanSlot.label`，而注射槽位 label 被写死为「注射」；现抽出共用 `PlanTimePicker`（口服多选 / 注射单选），字段改名「计划用药时间（到点提醒）」，注射槽位 label 改为计划时刻，并修正不实的「默认 09:00」说明）** + **v1.0.52「计划用药时间」改用真正的时间选择器（无结构变更：删除固定候选 chip 与「自定义 HH:mm」输入框，改用 M3 `TimePicker`（24h）；当前已设时刻**始终以文本显示**——旧实现用 chip 选中态表达当前值，存的时刻不在那 5 个候选里（如 07:30）就完全看不出来；新增 `ScheduleCalc.timeParts()` 容错解析选择器初始值；默认时刻统一为唯一常量 `DEFAULT_PLAN_TIME`，此前表单 08:00 与 `slotsFor` 兜底 09:00 不一致）** —— v1.0.52 曾为正式版（2026-09-23 用户真机确认后转正），现为历史版本；v1.0.44~v1.0.51 仍为预发布且各含「已被 v1.0.52 取代」横幅，仅作历史留存；v1.0.38~v1.0.41 那批废弃版未转正（v1.0.39 冷启动必崩，切勿标为正式版） + **v1.0.53 S1 落地：提醒链首次有自动化回归测试（无结构变更：引入 Robolectric 4.13 仅测试期依赖 + `ReminderSchedulerTest` 9 条锁「取消-重建」语义；为让测试任何时刻运行都确定，`rescheduleAll` / `cancelAllFuture` 增加带默认值的 `now` / `today` 参数——**这是本版唯一的生产代码改动**；S1b 滑杆手势测试已实测证伪、不予落地）** + **v1.0.54 趋势页方案 C：2 列小多图同屏 + 共享时间轴 + 新增 ESR / CRP 客观炎症指标（无结构变更：新 `domain/LabIndicator` 指标目录（名称归一 + 单位换算，刻意不做模糊匹配）+ `domain/LabTrends` 纯函数（单位换算/窗口/同日去重三类口径 + 未纳入条数如实计数）+ `ui/components/SmallTrendChart` 小多图单元；炎症阈值进 `ClinicalThresholds.ESR_HIGH/CRP_HIGH` 并优先用化验单自带 `refHigh`；点任一格可展开回 v1.0.45 的大图，不丢拖动读数）** —— v1.0.53 曾为正式版（2026-09-24 用户确认后转正），**已于 v1.0.58 转正后转为历史版本**；v1.0.52 亦为历史版本并带取代横幅；v1.0.44~v1.0.51 仍为预发布且各含取代横幅，仅作历史留存；v1.0.38~v1.0.41 那批废弃版未转正（v1.0.39 冷启动必崩，切勿标为正式版） + **v1.0.55 修 v1.0.54 炎症指标四问题（无结构变更：① 化验不再套 7/30/90 天窗口（季度采样，套 30 天几乎永远为空）——改 `LabResultDao.allOrdered()` + `LabTrends` 窗口参数改可选；② 日常与化验**两套独立共享时间轴**（采样频率差 1~2 个数量级，同轴必然压扁一方）；③ 指标名识别改「有界变形（剥尾限定词 / 去括号）后命中别名表」——v1.0.54 靠枚举漏了真实数据「红细胞沉降率测定」；④ 空格子文案按分组传入（收缩压 / 心率 曾误显示「暂无化验数据」）；⑤ 格子标题只用中文名，缩写移到大图标题）** —— **v1.0.55 为当前预发布（含 v1.0.44~v1.0.54 全部内容），待真机确认后转正** + **v1.0.56 修「hs-CRP 并入 CRP 导致数值画错」（无结构变更：hs-CRP 独立成第三项指标 `HSCRP`（CRP 别名表移除全部超敏写法，两者互不匹配、阈值各取自己那份 refHigh）；新增 `LabIndicator.alwaysShow`——ESR/CRP 恒占格、hs-CRP 仅在有数据时占格；同日不同数值新增 `LabTrend.sameDateConflict` 计数并在图下如实说明，不再静默取其一；`allOrdered()` 排序补 `recorded_at, id` 使选择可复现）** —— **v1.0.56 为当前预发布（含 v1.0.44~v1.0.55 全部内容），待真机确认后转正** + **v1.0.57 真修「同日多值画错」（无结构变更：去掉「同日取最近录入一条」规则——**同日多个不同数值全部画出、一个不丢**（完全相同值才合并）；`LabTrend.sameDateConflict` 改为 `conflictDates`（日期数），文案改为「有 N 个日期存在多条不同数值（均已画出）」；**移除 hs-CRP**（用户要求）——`LabIndicator.HSCRP` 与 `HSCRP_HIGH` 删除、`alwaysShow` 一并删除，CRP 别名表不含超敏写法）** + **v1.0.58 拖动读数与说明文案收尾（无结构变更：① **拖动读数不再只报一条**——`nearestIndex` 用严格小于比较、命中同日**第一个**下标，而同日多值在 `points` 里按数值升序，于是永远报到较小的 0.4（用户实测「手指拖动到 3.13 还是显示 0.4」）；新增纯函数 `sameDateIndices` 一次取出该日**全部**下标，高亮与读数都不再只报一条，气泡文案由 `selectionText` 生成、同日多值全部列出**且从大到小**；② 撤掉 v1.0.57 加在小图下的那行「有 N 个日期存在多条不同数值（均已画出）」——那些值本就已全部画出、图上一段竖线已把话说清，多挂一行字只会把该格撑高、与左侧格不齐、破坏 2 列小多图的整齐；连带删除只服务于它的 `LabTrend.conflictDates` 与字符串 `report_lab_same_date_conflict`，`hasCaveat` 只保留「有数据被排除在外」）** —— **正式版（Latest）为 v1.0.58（2026-09-25 用户真机确认后转正）**；v1.0.54 ~ v1.0.57 仍为预发布且各含取代横幅（v1.0.57 的横幅已改为「已被 v1.0.58 取代」），仅作历史留存 + **v1.0.59 B5 多源提醒（无结构变更：三源独立 Scheduler——`CheckupReminderScheduler`（复诊提前1天+当日 09:00，无升级链）/ `BasdaiReminderScheduler`（评估日 20:00 + +1/+2 天 20:00 升级，周期可配 7/14/28/56/84 天）/ `ExerciseReminderScheduler`（18:00 + 20:00/22:00 升级，今日已打卡则不排）；3 个新通知通道（`checkup_reminders` HIGH / `questionnaire_reminders` DEFAULT / `exercise_reminders` DEFAULT）；requestCode 命名空间 `chk|`/`bas|`/`exc|` 防与用药撞，通知 ID 前缀 `n_chk|`/`n_bas|`/`n_exc|`；3 个新 Receiver 验真 extra 后发通知（不在 Receiver 内排下一级，升级链在 rescheduleAll 时全部预排）；`ReminderConfigRepository` 管理 `reminder_config` prefs（非密钥 prefs，不跨备份恢复）；`BootReceiver` / `AshkbApplication` 启动期 + 4 个 ViewModel 数据变更点（saveCheckupRecord / saveBasdai / checkIn / activate-deactivate）即时 reschedule；「我的」页新增「提醒设置」卡（`ReminderSettingsSheet`：复诊/运动开关 + BASDAI 周期单选，即时写入+重排）；18 条 Robolectric 单测覆盖 7 天窗口/升级链/幂等/取消语义）** —— **正式版（Latest）为 v1.0.59**；v1.0.58 转为历史版本 + **v1.0.60 B8 免打扰时段 + 同时段多提醒合并推送（无结构变更：`domain/DndWindow.kt` 跨午夜纯函数（左闭右开 / start==end 视为全天）+ `ReminderConfigRepository` 加 `dnd_enabled`/`dnd_start`（默认 22:00）/`dnd_end`（默认 07:00）；免打扰走**静默投递**而非顺延——`NotificationHelper.postXxxReminder` 加 `silent` 参数，静默走新通道 `reminder_silent`（IMPORTANCE_LOW / 无声无振 / 通知栏仍可见），4 个 Receiver 触发时调 `NotificationHelper.isInDndNow(context)` 实时读配置（开关与时间变更**只写 prefs、无需重排闹钟**）；同时段合并推送——4 类提醒全部 `setGroup(ashkb_reminders)`，`updateGroupSummary` 在 2+ 条时发 `GROUP_ALERT_SUMMARY` 折叠通知（「您有 N 条待处理提醒」）、不足 2 条时撤掉 summary，post 与 cancel 两路都调用；`ReminderSettingsSheet` 加免打扰开关 + 起止时间（系统 `TimePickerDialog`，`SettingSwitchRow` 扩 subtitle 位）；12 条 `DndWindowTest` 单测（跨午夜/同日/边界/全天））** —— **正式版（Latest）为 v1.0.60**；v1.0.59 转为历史版本 + **v1.0.61 B9 升级链第三级「强提醒」（无结构变更：用药链末级（esc==`MAX_ESCALATION`）改用 `fullScreenIntent` 拉起新增 `ReminderFullScreenActivity`（`showWhenLocked` + `turnScreenOn` + `taskAffinity=""` 独立任务栈不进最近任务，API 27 边界守卫；界面含药名/剂量/计划时间 + 「已服用」（免开应用写库打卡）/「稍后处理」）；`ReminderScheduler.isStrongEscalation(escalation)` 纯函数判定末级；`NotificationHelper.postMedReminder` 加 `strong` 参数 + `setFullScreenIntent`，文案换 `notif_med_strong_*`；仅用药启用（BASDAI/运动非紧急不升级），DND 静默时段内不升级全屏（`strong && !silent` 才生效）；Manifest 加 `USE_FULL_SCREEN_INTENT`，Android 14+ 默认不授予 → 「提醒可靠性自检」卡加「强提醒（全屏）」行 + `ACTION_MANAGE_APP_USE_FULL_SCREEN_INTENT` 引导按钮，未授予优雅降级为横幅；单测 +1 条）** —— **正式版（Latest）为 v1.0.61**；v1.0.60 转为历史版本 + **v1.0.62 C11 提醒可靠性收口（无结构变更：电池白名单——自检卡用 `PowerManager.isIgnoringBatteryOptimizations` 显真实状态 + 「加入电池白名单」一键跳（官方直连弹窗优先，兜底电池优化列表页），Manifest 加 `REQUEST_IGNORE_BATTERY_OPTIMIZATIONS`；自启动——新增 `reminder/SystemSetupGuides.kt` 按厂商（小米/华为/荣耀/OPPO/一加/realme/vivo/iQOO/三星/魅族）组件名尽力跳转 + 兜底应用详情页，**刻意不做状态行**（系统无公开查询接口）；测试提醒——新增 `reminder/ReminderTest.kt`（10 秒后精确闹钟，`nowMs` 可注入）+ `TestReminderReceiver` + `NotificationHelper.postTestReminder`（走 `sys_notices`，不归入折叠组），端到端验「权限 + 精确闹钟 + 通道 + 投递」，DND 时静默；另修「自检卡从系统设置页返回后状态不刷新」——`ON_RESUME` 观察者触发重读；动作按钮横排 Row 改纵向满宽 Column（最多 4 个会溢出）；单测 +4 条）** —— **正式版（Latest）为 v1.0.62**；v1.0.61 转为历史版本 + **v1.0.63 C12 免责声明收口（无结构变更：首启声明门禁——新增 `ui/DisclaimerScreen.kt` 的 `FirstLaunchDisclaimer` 全屏页，`AppShell` 顶部早返回（`rememberSaveable` + `data/repo/DisclaimerStore.kt` 存 `app_prefs.disclaimer_accepted`），**确认前不创建任何 ViewModel / 不打开数据库**；四条要点：非医疗建议/不替代诊疗 · 用药与调整以主治医师医嘱为准 · 数据仅存本机不上传 · 不是医疗器械；统一前缀——唯一文案来源 `domain/Disclaimer.kt`（`PREFIX`「仅供参考，以主治医师医嘱为准。」+ 4 条声明 + `FIRST_LAUNCH_POINTS`，与 `RecipeSources.DISCLAIMER` 同理放 domain 以便单测锁措辞）+ 共享组件 `ui/components/DisclaimerNote.kt`，应用于三处自动生成提示（漏服处理 / 复诊准备 / 跨院化验）并收敛各自重复措辞（`missed_dose_disclaimer` / `checkup_prep_disclaimer` / `lab_unit_group_hint`）；**条目级（知识库 / 食谱）与 PDF 页脚刻意保持自足完整文本**——要脱离 App 被阅读；`DisclaimerTest` 6 条）** —— **正式版（Latest）为 v1.0.63**；v1.0.62 转为历史版本 + **v1.0.64 B13 生活方式画像（无结构变更，复用闲置的 `profile.lifestyle` 列：新增 `domain/Lifestyle.kt`（扁平 JSON 手写编解码——零第三方依赖 + domain 纯 JVM 拿不到 `org.json`；`hasSmokingHistory` / `pinnedKbIds()`）+ `domain/LifestylePrescription.kt`（画像→个性化提示：戒烟 / 久坐每 30–45 分钟起身 / 睡眠不足不宜过量 / 无习惯从 L1 起步 / 有习惯按进展原则加量；**刻意只出提示、不改 R27 红黑榜过滤**——过滤依据是分期与活动度，把吸烟/久坐/睡眠塞进过滤器属无依据的医学决策）；`ProfileEditScreen` 补采集（吸烟 / 运动习惯 chips + 久坐 / 睡眠数字输入），`MeScreen` 档案卡加「生活方式」一行摘要；`ExerciseUiState` 加 `lifestyleNotes`，运动页 hero 在判读依据下新增「生活方式提示」块并复用 `DisclaimerNote`；**顺带修掉 §D 最后一项悬空挂点**——`KnowledgeViewModel.uiState` 由 4 路 combine 扩为 5 路（并入 `observeProfile()`），登记吸烟后「全部」视图置顶 `edu-003`（检索 / 分类视图不置顶）；`LifestyleTest` 7 + `LifestylePrescriptionTest` 8）** —— **正式版（Latest）为 v1.0.64**；v1.0.63 转为历史版本 + **v1.0.65 B12 极简模式状态机（⚠️ **含结构变更 Room v15→v16**——本版起不再是「无结构变更」：`profile` 加 `minimal_since TEXT`（`MIGRATION_15_16`）。红线三 e2：新增 `domain/MinimalMode.kt` 纯函数状态机（`THRESHOLD_DAYS=3` / 连续缺失计数**封顶为阈值** / `entersMinimal` 只认身体不适与住院 / `isConsistent` 锁 ui_mode 与 minimal_since 的成对不变式）+ `data/repo/MinimalPromptStore.kt`（`app_prefs` 记「今日已问」）；`HealthRepository` 加 `symptomDatesBetween` 与 `setMinimalMode`（成对写入）；`TodayViewModel` 加 `app` 入参与 `minimalPrompt` 流 + `answerMinimalPrompt`/`exitMinimalMode`；今日页加极简横幅（自 YYYY-MM-DD + 退出按钮）与原因询问弹窗（**不给点外部关闭**），极简态隐藏运动快捷入口、保留告警与用药；**核心记录口径拍板为 `symptom_daily`**（不选用药打卡：无在用药品时天然为空会误判）；`ProfileEditScreen` 补透传 `minimalSince`（否则编辑档案会写成不一致状态）；备份引擎无需改（表名 `sqlite_master` 动态、列名 `PRAGMA table_info` 动态校验）；`MinimalModeTest` 8）** —— **正式版（Latest）为 v1.0.65**；v1.0.64 转为历史版本 + **v1.0.66 B6a 锁屏紧急信息（无结构变更：紧急卡页新增「锁屏显示紧急信息」开关（**默认关闭**，`app_prefs` 存 `emergency_lockscreen_enabled`）；新增 `domain/EmergencyLockscreen.kt` 纯函数文案构建（过敏 JSON 数组展平为顿号 / 免疫抑制类标注感染风险 / 用药封顶 `MAX_MED_LINES=4` 并报总数 / 家属优先于医生 / 非紧急联系人不进锁屏）+ `data/repo/EmergencyLockscreenStore.kt` + `reminder/EmergencyLockscreenPublisher.kt`（读库→构建→投递；开关关或无可显示内容则撤下通知）；`NotificationHelper` 加通道 `emergency_lockscreen`（IMPORTANCE_LOW / 锁屏公开可见 / 不响不震）与 `postLockscreenEmergencyCard`（`VISIBILITY_PUBLIC` + `setOngoing` + `setOnlyAlertOnce`）/`cancelLockscreenEmergencyCard`；`AshkbApplication` 启动期同步一次、`EmergencyScreen` 页面内数据变化即时同步（**不做后台轮询**：无后台服务与 WorkManager）；**为何用常驻通知**——Android 无面向普通应用的自定义锁屏控件（锁屏 Widget 已废弃），`VISIBILITY_PUBLIC` 通知是唯一受支持途径；`EmergencyLockscreenTest` 9）** —— **正式版（Latest）为 v1.0.66**；v1.0.65 转为历史版本 + **v1.0.67 C1 骶髂关节影像分期（⚠️ **含结构变更 Room v16→v17**：`profile` 加 `sacroiliitis_grade TEXT`（`MIGRATION_16_17`）。补规划 M0「诊断信息全量」缺的最后一项：`Labels` 加 `SACROILIITIS_KEYS`（"0".."4"）+ `sacroiliitisGrade()`（「0 正常 / I 可疑 / II 轻度 / III 中度 / IV 重度」，罗马数字在标签内、key 不进 UI）；建档表单 chips（未评估 + 0–IV）+ 「我的」档案卡 + **复诊报告 PDF** 基本信息区；**刻意不进打印版应急卡与锁屏卡**（急救场景只要即刻信息）；**`BackupRepository` 的档案 JSON 导入导出是独立字段清单**，双方向同步补该字段（否则导出再导入会丢）；`LabelsTest` 4）** —— **正式版（Latest）为 v1.0.67**；v1.0.66 转为历史版本
...
- **待办池**（用户视角，无承诺）：
  ...
  - **S1 提醒链回归测试（Robolectric）—— ✅ 已于 v1.0.53 落地**：`app/src/test/java/com/ashkb/app/reminder/ReminderSchedulerTest.kt`，9 条，断言「闹钟触发时刻集合」这一对外唯一可观察效果（未来 7 天各排首次提醒 / 今日已过点未打卡**会**重建 +30 与 +60 / **已打卡**的槽位**不再**重建（N1 回归锁）/ 未到点的不排升级重查 / 重复重排幂等 / `cancelAllFuture` 能**精确清空** `rescheduleAll` 排出的全部闹钟（取消-重建对称性）/ 停药显式取消后不留残留（R6）/ Q2W 只在注射日排 / `slotRef` 维度）。基建：`testImplementation("org.robolectric:robolectric:4.13")` + `testOptions { unitTests { isIncludeAndroidResources = true } }`，**instrumented `android-all(API 34)` 与传递依赖均已在本机缓存，可离线跑**。为让测试**任何时刻运行都确定**，给 `rescheduleAll` / `cancelAllFuture` 加了带默认值的 `now` / `today`（原因见 §7）。
    - 若要扩展本类：只在 `@Before` 里 `ShadowAlarmManager.setCanScheduleExactAlarms(true)`（否则走 `setWindow` 分支），并**务必显式传 `now`**——不要新写「相对当下」的构造。
  - **S1b 打分滑杆手势回归测试（Robolectric + `compose-ui-test`）—— ❌ 已实测证伪，最终不落地（v1.0.53）**：`ScoreInput` 的「拖动失效」横跨 29 个版本没被任何测试发现，因为它是**手势命中行为**。但 v1.0.53 实测证明本环境**测不了这件事**：能注入触摸（最小 `pointerInput` 盒子收得到 down），但**裸 M3 `Slider` 也拖不动**（对照探针，值与 ScoreInput 无关地恒为 0）。故那 3 条断言是**假红**，落地只会变成「要么永远失败、要么被删掉」的噪声。**滑杆手势仍只能真机验证**；`build.gradle.kts` 与 §7 均已就地记明，勿再起这个坑。
  - **趋势页方案 B / C（2026-09-21 用户选「先只做 A」）**：A（精准修复 + 时间范围切换）已于 v1.0.45 落地；**C 已于 v1.0.54 落地**。剩余一项待评估：
    **B「总览 + 展开」**——趋势页顶部一张「指标总览」卡（每个指标一行：名称 + 当前值 + 变化 + sparkline），点行进入该指标的详情大图。解决「8 张图要滚很久」，代价是看细节多一次点击。**注**：v1.0.54 的方案 C 已实现「点格展开大图」（ModalBottomSheet 复用 `TrendChart`），B 若要继续，应基于此收敛——否则两套「看详情」入口会重复。
    ~~**C「小多图 + 炎症指标」**~~ —— **已落地（v1.0.54）**：改为 **2 列 × 4 行共 8 格**（原计划 2×3 只算了 6 个指标，加上 ESR/CRP 共 8 个），共享时间轴由「全部序列日期并集」算出；新增 `domain/LabIndicator`（指标名归一 + 单位换算）、`domain/LabTrend`（化验行 → 序列）、`Trends.labs` 字段。**
...
| B5 ✅ | 多源提醒（运动 / BASDAI 问卷 / 复诊）——**已于 v1.0.59 落地**（三源独立 Scheduler + 3 通道 + ReminderSettingsSheet + 18 单测；无结构变更） | `ReminderScheduler.rescheduleAll(context, meds)` 只遍历药单；全部调用点只传药单 |
| B6 🔶 | 紧急卡「锁屏紧急信息集成」**✅ 已于 v1.0.66 落地**（`EmergencyScreen` 开关（默认关闭）+ `emergency_lockscreen` 常驻通道（IMPORTANCE_LOW / 锁屏公开可见 / 不响不震）+ `NotificationHelper.postLockscreenEmergencyCard`（`VISIBILITY_PUBLIC` + `setOngoing`）+ `domain/EmergencyLockscreen.kt` 纯函数文案（过敏 JSON 展平 / 免疫抑制标注 / 用药封顶 4 条报总数 / 家属优先 / 非紧急联系人不进）+ `reminder/EmergencyLockscreenPublisher.kt`（启动期 + 页面内数据变化即时同步；开关关或无内容则撤下）+ `EmergencyLockscreenTest` 9）；**「钥匙扣二维码模板」仍未做**——需自写 QR 编码器（零第三方依赖），留作独立增量 | R19 三项只落打印版 PDF；`grep 锁屏` 此前零命中 |
...
| B8 ✅ | 免打扰时段 + 同时间段多条提醒合并推送——**已于 v1.0.60 落地**（`domain/DndWindow.kt` 跨午夜纯函数 + 静默通道 `reminder_silent` + 通知组 `ashkb_reminders` summary + 设置面板开关/起止时间；Receiver 实时读配置，无需重排闹钟；12 单测） | M10；零实现 |
| B9 ✅ | 提醒升级链第三级「强提醒（全屏 / 横幅）」——**已于 v1.0.61 落地**（用药链末级改用 `fullScreenIntent` + `ReminderFullScreenActivity`（showWhenLocked/turnScreenOn）；Android 14+ 需 `USE_FULL_SCREEN_INTENT`，自检卡给授权引导，未授予降级为横幅；仅用药启用，DND 时不升级全屏） | v3 M10；无 `fullScreenIntent`，`MAX_ESCALATION` 到 2 即止 |
...
| ~~B12~~ | ~~极简模式状态机（连续 3 天核心记录未完成 → 询问原因 → 身体不适/住院切极简模式）~~ —— **已于 v1.0.65 落地**（红线三 e2。**Room v15→v16**：`profile` 加 `minimal_since`；`domain/MinimalMode.kt` 纯函数状态机——**核心记录口径拍板为 `symptom_daily`**（唯一「每日必填且与用药清单无关」的自评入口；刻意不选用药打卡——无药时天然为空会误判）；连续缺失计数**封顶为阈值**；只有「身体不适/住院」进极简、`other` 不改界面（否则掩盖真实数据缺口）；`ui_mode` 与 `minimal_since` 成对维护（`isConsistent` 断言 + `ProfileEditScreen` 补透传）；今日页横幅 + 退出按钮，极简态隐藏运动快捷入口、保留告警与用药；`MinimalPromptStore` 同日只问一次；备份引擎无需改（表/列白名单均动态发现）；`MinimalModeTest` 8） | 红线三；`Profile.ui_mode` 字段空转（仅 Entity 定义处），`minimal_since` 字段不存在，无判定 / 询问 / 切换 |
| ~~B13~~ | ~~生活方式画像（吸烟 / 职业久坐时长 / 运动习惯 / 睡眠）→ 驱动 M4 处方个性化~~ —— **已于 v1.0.64 落地**（`profile.lifestyle` 此前只透传未采集 → 建档表单补采集 + 「我的」显示摘要；`domain/Lifestyle.kt`（扁平 JSON 手写编解码）+ `domain/LifestylePrescription.kt`（画像→个性化提示，**只出提示不改红黑榜过滤**——过滤依据是分期与活动度）；运动页新增「生活方式提示」块并复用 `DisclaimerNote`；登记吸烟后知识库置顶 `edu-003`，**顺带修掉 §D 的悬空挂点**；`LifestyleTest` 7 + `LifestylePrescriptionTest` 8） | M0；`Profile.lifestyle` 字段空转，未采集未展示 |

**C. 弱化实现（有字段或半套流程，未达规划口径）**

| # | 缺口 | 现状 vs 规划 |
|---|---|---|
| ~~C1~~ | ~~骶髂关节影像分期~~ —— **已于 v1.0.67 落地**（⚠️ **Room v16→v17**：`profile` 加 `sacroiliitis_grade`（改良纽约标准 mNY X 线 0–IV）；`Labels` 加 `SACROILIITIS_KEYS` + `sacroiliitisGrade()`（含罗马数字，key 不进 UI）；建档表单 chips（未评估 + 0/I/II/III/IV）+ 「我的」档案卡展示 + **复诊报告 PDF** 基本信息区；**刻意不进打印版应急卡与锁屏卡**（急救只要即刻信息，分期属慢性期资料）；`档案 JSON 导入导出`（`BackupRepository` 独立字段清单）双方向同步补 `sacroiliitis_grade`；`LabelsTest` 4） | 规划 M0 要求（诊断信息全量），`Profile` 无该字段 |
...
| ~~C12~~ | ~~全局免责声明（显著位置 / 首启声明）+ 自动提示统一前缀「仅供参考，以主治医师医嘱为准」~~ —— **已于 v1.0.63 落地**（首启全屏门禁 `FirstLaunchDisclaimer`，确认前不创建任何 VM / 不开库，状态存 `app_prefs`；统一前缀唯一来源 `domain/Disclaimer.kt` + 共享组件 `DisclaimerNote`，应用于漏服 / 复诊准备 / 跨院化验三处自动提示并收敛各自重复措辞；条目级与 PDF 页脚保持自足文本；`DisclaimerTest` 6 条锁措辞） | 现仅条目级与 PDF 页脚声明 |
...
> ✅ **已全部了结（v1.0.64）**，本节可作历史留存：唯一剩余的 `edu-003`（吸烟条目）挂点选了「补齐实现」——v1.0.64 起 `profile.lifestyle` 真正采集，登记吸烟后知识库「全部」视图将 `edu-003` 置顶（见 §8 B13）。**知识库种子挂点已无悬空项。**
...
- **测试基线**：**471 条单测全绿**；新增功能须同步补测（`app/src/test/.../`，**42 个测试文件**覆盖 backup / 加密（v2+v3）/ 附件路径与远端比对 / PROPFIND 解析 / 停药警示与三态 / 化验单位分组 / 筛查种子 / 补剂上限与错开 / 食谱种子与出处台账 / 计划模板编解码 / 计划完成度 / 通用名键 / 运动分级 / 导入解析 / 排程计算 / 知识库检索 / 紧急卡用药汇总 / 恢复码 / 复诊准备 / 漏服 / 体重目标 / **服药依从口径与用药记录修正不变量 / 注射部位存库键稳定性 / ISO 时刻与时间选择器口径（`hhmm`、`timeParts`、槽位标签 = 计划时刻）/ 崩溃留档凭据脱敏 / 源码正则花括号静态守卫 / 提醒链「取消-重建」语义（Robolectric，含末级强提醒判定）/ 测试提醒排程（Robolectric：延迟换算 / 幂等 / 取消）/ 免打扰时段判定（跨午夜 / 同日 / 左闭右开边界）/ 免责声明措辞 / 生活方式画像编解码与置顶挂点 / 生活方式→处方提示 / 极简模式状态机（连续缺失封顶 / 阈值边界 / 原因映射 / 成对不变式）/ 锁屏紧急卡文案（JSON 展平 / 免疫抑制标注 / 封顶报总数 / 联系人优先级）/ 档案标签映射（骶髂关节分期 0–IV 与未评估兜底）/ 炎症指标名称归一与单位换算 / 化验行→序列的三类口径 / 小多图共享时间轴 / 趋势图选中读数（同日多值全取 + 气泡从大到小）**）
...
| v1.0.49 | ①**修正记录**：我的 → 药单 → 点药名 → 用药记录 → 点某条右侧**铅笔** → 改状态（已服 / 部分 / 跳过）→ 保存 → 列表应立即变（含顶部依从率随之变）②**改状态要连带清字段**：把一条「跳过」改成「已服」→ 该行**不应再显示原因**；把一条「已服」改成「跳过」→ 必须选原因才能保存（不选则「保存」置灰）③**注射部位中文**：注射类药的记录里应显示「左大腿 / 右腹部」等中文，**不应出现 `thigh_l` 这类英文键** ④**部位选择**：注射记录改成「已服」时出现部位 chips，选一个保存后列表显示对应中文 ⑤**归属日与计划时刻不可改**：弹层里这两项是只读的（只作为标题与说明出现）⑥**同一时间窗依从率一致**：药单弹层 ↔「报表 → 概览 → 服药依从」数值应一致（同一公式）⑦**已停用药品**区与全部停用时的显示（v1.0.48 项）仍正常 ⑧打分滑杆拖动（v1.0.47 项）仍正常 ⑨旧备份恢复后数据完整（无结构变更） | ⚠️ **未真机验；已被 v1.0.50 取代**（功能全部含于 v1.0.50），**请直接验 v1.0.50** |
| v1.0.50 | ①**今日页「已服」后的时刻**：打卡一条药 → 今日页该卡应显示「已服 **HH:mm**」（如「已服 22:31」）——**不应再出现「已服 19981」这类一串数字**（旧版把小数秒当成了时刻）②**晚于计划**：若晚于计划 30 分钟以上，后面仍应跟着「（晚于计划 30 分钟以上）」③**补剂历史时刻**（营养页点补剂 → 近 90 天记录）应仍显示「2026-09-23 22:31」形式（该处走 `take(16)`，本就正常，顺带确认没被改坏）④用药记录修正 / 已停用药品（v1.0.49 / v1.0.48 项）仍正常 ⑤打分滑杆拖动（v1.0.47 项）仍正常 ⑥旧备份恢复后数据完整（无结构变更） | ⚠️ **未真机验；已被 v1.0.51 取代**（功能全部含于 v1.0.51），**请直接验 v1.0.51** |
| v1.0.51 | ①**注射类能设计划用药时间**：我的 → 药单 → 添加（或编辑）一支**注射**药 → 选频次（如「每两周一次」）→ 应出现**「计划用药时间（到点提醒）」**并可选 06:30 / 08:00 / 12:00 / 18:00 / 21:00 或自定义 → 保存 ②**注射单选**：再点已选中的时刻不应取消（避免变成「零个时刻」）③**今日卡显示时刻**：注射日当天，今日卡上应显示该**计划时刻**（如「08:00」），旁边仍有「注射」标签；**不应只有一个「注射」看不到时刻** ④**口服仍可多选**：编辑一支口服药（如每日两次）→ 仍能选 08:00 + 20:00，并显示「已选时刻：08:00、20:00」⑤**自定义时刻校验**：输入 `25:99` 点「添加」应不生效（此前会被加进去但存库后被过滤，等于假生效）⑥**文案**：注射表单里不应再写「默认 09:00 提醒」⑦用药记录修正 / 已停用药品（v1.0.49/48 项）仍正常 ⑧今日页「已服 HH:mm」仍正常（v1.0.50 项）⑨旧备份恢复后数据完整（无结构变更） | ⚠️ **未真机验；已被 v1.0.52 取代**（功能全部含于 v1.0.52），**请直接验 v1.0.52** |
| v1.0.52 | ①**时间选择器**：我的 → 药单 → 编辑一支**注射**药 → 「计划用药时间（到点提醒）」下**只有一行当前时刻**（如 `09:00`）→ 点它弹出**时间表盘**（24 小时制）→ 选任意时刻（如 07:30）→ 保存 → 再进编辑页应显示 `07:30` ②**不再有固定候选**：界面上不应再出现 06:30 / 08:00 / 12:00 / 18:00 / 21:00 那排 chip，也不应有「自定义 HH:mm」输入框 ③**表盘可完整显示**：在真机上确认表盘**完整可见、可拖动**（窄屏 ≤320dp 重点看；若偏挤请反馈，下版改底部弹层）④**口服多选**：编辑一支口服药（如每日两次）→ 已选时刻逐个列出 → **点某个时刻可改**、**点 ✕ 可删**、**「＋ 添加时刻」可新增** ⑤**老值可见**：若某支药的时刻不在常用候选内（如 07:30），编辑页必须**显示出来**（旧版会完全不显示）⑥**注射仍是单选**：不会出现「零个时刻」⑦今日卡仍显示计划时刻（v1.0.51 项）、今日页「已服 HH:mm」仍正常（v1.0.50 项）⑧旧备份恢复后数据完整（无结构变更） | ☑ **已验（2026-09-23 用户确认「都没问题了」）**，**已转正为当前正式版（Latest）** |
| v1.0.54 | ①**小多图同屏**：报表 → 趋势 → 应看到 **2 列共 8 格**（BASDAI / 疼痛 / 晨僵 / 体重 / 收缩压 / 心率 + 炎症指标区 血沉 ESR / C 反应蛋白 CRP），无需长滚动即可同屏比较 ②**共享时间轴**：顶部应有一行「各图共用同一时间轴：起 → 止」；**同一天的记录在各格中应落在水平方向的同一位置**（这是本版最关键的一点，也是最容易看错的一点）③**点格展开**：点任一格 → 弹出大图（可拖动读数、有读数摘要）→ 下滑关闭 ④**炎症指标有数据**：若化验单里导入过「血沉(ESR)」「C-反应蛋白(CRP)」，应出现折线与末值；**阈值虚线**应画在化验单自带的参考上限处（若化验单没带，则用 20 mm/h / 8 mg/L）⑤**别名归一**：指标名为「ESR」「血沉」「血沉（ESR）」「超敏C反应蛋白」的记录都应对上同一格，**不应出现两条分开的线或漏画**⑥**单位换算**：若某次 CRP 记的是 `mg/dL`，其值应按 ×10 折到 `mg/L` 后入图（不应画成「骤降」）⑦**不静默丢弃**：若存在单位认不出/缺失的记录，图下应显示「另有 N 条因单位不同未纳入 / 无单位按 … 计」⑧**无化验数据时**：炎症指标两格应显示「暂无化验数据」及导入入口提示（不是整格消失）⑨时间范围切换（7/30/90 天，v1.0.45 项）仍正常 ⑩旧备份恢复后数据完整（无结构变更） | ⚠️ **已被 v1.0.55 取代**（用户实测发现化验套 7/30/90 天窗口不可用 + ESR 漏配），**请直接验 v1.0.55** |
| v1.0.55 | ①**血沉 ESR 应出图**：复诊管理→化验里存的名字是「红细胞沉降率测定」→ 趋势页炎症区第一格**必须画出折线与末值**（v1.0.54 此处永远是空的）②**化验不受时间范围限制**：切「近 7 天 / 近 30 天 / 近 90 天」→ 炎症区两张图与「本区时间轴」行**应保持不变**（化验是季度采样，本就不该被 7/30/90 天裁剪）；普通指标 6 格则应随范围变化 ③**日常指标不被化验压扁**：若化验记录跨了 1 年、而症状只记了近 30 天，普通指标 6 格**不应**被挤成左侧一条线（两区各自一根时间轴）④**空格子文案正确**：完全没用过的指标（如收缩压 / 心率 / 体重）空格子应显示「**暂无数据**」，**不应**显示「暂无化验数据」（那是化验专用文案）⑤**格子标题不被截断**：炎症格的标题应完整显示「血沉」「C反应蛋白」，**不应**出现「C反应蛋白 C...」这种被数值挤掉的情况 ⑥**点开大图标题带缩写**：点炎症格 → 大图标题应显示「血沉 ESR」/「C反应蛋白 CRP」⑦单位换算式不静默丢弃（v1.0.54 项）仍正常 ⑧旧备份恢复后数据完整（无结构变更） | ⚠️ **已被 v1.0.56 取代**（2026-03-13 那类「图 0.4 / 化验单 36.33」见 v1.0.56），**请直接验 v1.0.56** |
| v1.0.56 | ⚠️ **本版修的是「数值画错」，务必核对数值本身**：①**CRP 数值必须与化验单一致**：复诊管理→化验→找到 2026-03-13 那条「C反应蛋白(CRP) 36.33 mg/L」→ 趋势页 C反应蛋白格的**03-13 点必须是 36.33**（v1.0.55 此处画成 0.4）、轴上限不再是 6 而是 ~40、超阈值次数应含这一次 ②**hs-CRP 独立成格**：若同一份化验单还有「超敏C反应蛋白(hs-CRP)」，它应是**另一格**（标题「超敏C反应蛋白」），而不是混进 C反应蛋白 ③**hs-CRP 无数据时不占格**：若某人的化验单没有 hs-CRP，炎症区仍只有 血沉 / C反应蛋白 两格（不该多出永久空格） ④**同日冲突如实提示**：若同一指标同一天真有两条不同数值，图下应出现「同日有 N 条不同数值，已取最近录入的一条」（不应静默取其一） ⑤**正常项里的同名项不再干扰**：化验单折叠的「正常项」里若有其它 C反应蛋白 类项目，不应把主曲线顶掉 ⑥前两版项（化验不受 7/30/90 限制 / ESR「红细胞沉降率测定」出图 / 空格子文案 / 标题不截断）仍正常 ⑦旧备份恢复后数据完整（无结构变更） | ⚠️ **已被 v1.0.57 取代**（数值仍画错：同日两条 CRP 只画了 0.4），**请直接验 v1.0.57** |
| v1.0.57 | ⚠️ **本版真正修的是「数值画错」，请以化验单为准逐点核对**：①**同日两条 CRP 都要画出来**：复诊管理→化验→2026-03-13 那两条 C反应蛋白(CRP)（36.33 与 0.4）→ 趋势页 C反应蛋白格在同一横坐标上应出现**两个点（一段竖线）**，**36.33 必须在图上**、纵轴上限 ~40、超阈值次数含这一次 ②**图下说明**：应显示「**有 1 个日期存在多条不同数值（均已画出）**」（不再是「已取最近录入的一条」）③**hs-CRP 已完全移除**：炎症区只有 血沉 / C反应蛋白 两格（不再出现「超敏C反应蛋白」格）；超敏类记录不参与任何曲线 ④**点开大图**核对 03-13 上方那个点是 36.33（拖到它看读数） ⑤**重复录入同值不受影响**：同一天两条相同数值只画一个点、且不显示「多条数值」提示 ⑥前几版项仍正常（化验不受 7/30/90 限制 / ESR「红细胞沉降率测定」出图 / 空格子文案 / 标题不截断 / 单位换算不静默丢弃）⑦旧备份恢复后数据完整（无结构变更） | ⚠️ **已被 v1.0.58 取代**（本版的①②③已含于 v1.0.58；但本版只把「同日多值都画出」做了一半——**拖动读数仍只报一条**，且小图下那行说明会撑高格子），**请直接验 v1.0.58** |
| v1.0.58 | ⚠️ **本版是 v1.0.57 的收尾，请与化验单核对数值**：①**拖动读数必须报出该日全部值**：复诊管理→化验→2026-03-13 那两条 C反应蛋白(CRP) 36.33 与 0.4 → 报表→趋势→点开「C反应蛋白」大图 → **手指拖到 03-13**：气泡应是「**03-13 · 36.3 / 0.4 mg/L**」（两个值都列出、**大的在前**；36.33 按全应用统一的 1 位小数显示为 36.3），**不能再只显示 0.4**；该处并应出现**两个圆点**（竖线两端各一个）②**小图下不再有说明行**：趋势页 C反应蛋白格下方**不应**再出现「有 N 个日期存在多条不同数值（均已画出）」；该格应与左侧「血沉」格**等高、顶对齐**（2 列小多图恢复整齐）③**同日两条 CRP 仍都要画**（v1.0.57 项）：小图上 03-13 处仍是一段竖线、**36.33 必须在图上**、纵轴上限 ~40 ④**「未纳入」的说明仍在**：若存在单位认不出的化验行，图下仍应显示「另有 N 条因单位不同未纳入」——这类是**图上看不出来**的，不能随②一起消失 ⑤前几版项仍正常（化验不受 7/30/90 限制 / ESR「红细胞沉降率测定」出图 / 空格子文案 / 标题不截断 / hs-CRP 已移除、炎症区只有 血沉 与 C反应蛋白 两格）⑥旧备份恢复后数据完整（无结构变更） | ☑ 已验（2026-09-25 用户确认「ok，转正式」，已转正为当前正式版 Latest） |
