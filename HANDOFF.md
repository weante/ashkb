...
> 记录截至 **v1.0.33**（versionCode 38，2026-09-21）的全部工程知识。
...
- **最新版**：v1.0.33（versionCode 38）：v1.0.21 知识库检索优化（Room v8→v9）+ v1.0.22 清除 `as MutableStateFlow` 强转 + v1.0.23 化验 Tab 分组折叠 + v1.0.24 通知/VM/PDF 文案资源化 + v1.0.25 Compose 性能审查第一批 + v1.0.26 紧急卡「当前用药」（A1）+ v1.0.27 备份恢复码（A2，v2 信封格式）+ v1.0.28 Compose 性能审查第二批 + v1.0.29 热修 v1.0.28 启动闪退 + v1.0.30 趋势图动画重播修复 + v1.0.31 药单在用药品编辑 + v1.0.32「我的」页关于卡 + **v1.0.33 规划缺口第二批（Room v9→v10：C4 复诊准备清单 / B2 知识库个人备注层 / C7 漏服处理指引 / C9 体重目标区间 / B10 复诊附件归档）**
