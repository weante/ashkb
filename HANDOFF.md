...
| v1.0.39 | ①「数据」页出现「推荐食谱」入口 → 进入见 10 条食谱，可按「抗炎 / 胃肠友好 / 控热量」筛选 ②点开一条食谱 → 详情见配料 / 做法 + 出处（编号 + 完整题录）+ 免责声明；**列表页不显示出处正文** ③收藏一条 → 置顶；自建一条 → 显示「自建」徽标且详情无出处小节 ④运动页出现「康复计划」入口 → 见 4 / 8 / 12 周三个模板 ⑤点「添加模板」→ 首次「已添加 3 个模板」，再点「模板已齐全」⑥启用 4 周计划 → 显示「第 1 / 4 周」「本周 x / 3 天」「累计 x / 16 天（y%）」；打卡运动后数字增加 ⑦旧备份恢复后数据完整（新表为空） | ⚠️ v1.0.39 **冷启动闪退**（已由 v1.0.40 热修），本行验证顺延至 v1.0.40 |
| v1.0.40 | ①**冷启动不崩**（v1.0.39 闪退修复）②「数据」→「推荐食谱」可用、10 条、筛选/收藏/自建正常 ③「运动」→「康复计划」可用、三模板 + 启用后进度正常 ④若本次启动仍异常，首屏会弹一条「上次启动异常：…」摘要（截图发给开发者）；完整堆栈在 `filesDir/last_crash.txt` | ◐ 部分已验（2026-09-21：**①冷启动不崩 ✓**（用户确认「不闪退了」）；②③ 待验）——**未转正式**（用户要求先只确认不闪退） |
