...
abstract class AppDatabase : RoomDatabase() {
    ...
    companion object {
        ...
        /** v6：P5 修订 R7——medications 新增 weekly_weekday2（BIW 每周两针第二星期） */
        private val MIGRATION_5_6 = object : Migration(5, 6) {
            override fun migrate(db: SupportSQLiteDatabase) {
                db.execSQL("ALTER TABLE `medications` ADD COLUMN `weekly_weekday2` INTEGER")
            }
        }
        ...
        private val MIGRATION_15_16 = object : Migration(15, 16) {
            override fun migrate(db: SupportSQLiteDatabase) {
                // v1.0.65 B12：极简模式记录进入时刻（与既有 ui_mode 列成对）
                db.execSQL("ALTER TABLE `profile` ADD COLUMN `minimal_since` TEXT")
            }
        }

        private val MIGRATION_16_17 = object : Migration(16, 17) {
            override fun migrate(db: SupportSQLiteDatabase) {
                // v1.0.67 C1：骶髂关节影像分期（改良纽约标准 0–IV）
                db.execSQL("ALTER TABLE `profile` ADD COLUMN `sacroiliitis_grade` TEXT")
            }
        }
